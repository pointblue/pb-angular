...
angular.module('material.components.slider')
    .directive('pbMdSlider', SliderDirective)
    .directive('pbMdSliderContainer', SliderContainerDirective);
...
function SliderDirective($$rAF, $window, $mdAria, $mdUtil, $mdConstant, $mdTheming, $mdGesture, $parse, $log, $timeout) {
    ...
    function postLink(scope, element, attr, ctrls) {
        ...
        if( angular.isDefined(attr.steps) && scope.steps.length > 0 ){
            //ignore min, max, and step attributes if steps is defined
            updateSteps( scope.steps );
            //assume the step values are in ascending order
        ...
        function stepValidator(value) {
            if (angular.isNumber(value)) {
                ...
                var formattedValue;
                if(steps){
                    formattedValue = findClosestStep( value );
                } else {
                    formattedValue = (Math.round((value - min) / step) * step + min);
                }
        ...
        function findClosestStep( value ){
            var diff;
            var lowestDiff = null;
            var lowestDiffIndex;
            var stepIsObject = steps.length && angular.isObject( steps[0] );
            var normalizedValue;
            for(var i=0;i<steps.length;i++){
                normalizedValue = stepIsObject ? steps[i].value : steps[i];
                diff = Math.abs( normalizedValue - value);
                if( ! angular.isNumber(lowestDiff) || (diff < lowestDiff) ){
                    lowestDiff = diff;
                    lowestDiffIndex = i;
                }
            }

            return stepIsObject ? steps[ lowestDiffIndex ].value : steps[ lowestDiffIndex ];
